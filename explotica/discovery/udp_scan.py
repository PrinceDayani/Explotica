...
@dataclass
class Evasion:
    """IDS/firewall-evasion knobs.

    source_port + data_length work in the privilege-free connected-socket
    engine. decoys + fragment require raw sockets (forging a source IP / IP
    fragments is impossible through the OS stack) and apply only in the raw tier.
    """
    source_port: Optional[int] = None   # bind probes to this src port (e.g. 53)
    data_length: int = 0                # append N random bytes to each payload
    decoys: tuple = ()                  # spoofed source IPs (raw tier only)
    fragment: bool = False              # IP-fragment probes (raw tier only)

    def pad(self, payload: bytes) -> bytes:
        if self.data_length > 0:
            return payload + os.urandom(self.data_length)
        return payload

    @property
    def needs_raw(self) -> bool:
        return bool(self.decoys) or self.fragment
...
def _probe_once(ip: str, port: int, payload: bytes,
    ...
    if evasion is not None:
        if evasion.source_port:
            # Trusted-source-port evasion (e.g. bind to 53). Best-effort:
            # concurrent binds to one port can conflict, so fall back silently.
            try:
                s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                s.bind(("" if family == socket.AF_INET else "::",
                        evasion.source_port))
            except OSError:
                pass
        payload = evasion.pad(payload)
...
def _scan_core(ips: list[str], ports: list[int], *,
    ...
    if deep:
        _deep_probes_all(states, default_timeout, evasion=evasion)

    return {ip: sorted(hs.resolved.values(), key=lambda p: p.number)
            for ip, hs in states.items()}


def _deep_probes_all(states: dict[str, HostState], timeout: float,
                     evasion: Optional[Evasion] = None) -> None:
...
def _maybe_raw(ips: list[str], ports: list[int], *, prefer_raw: bool,
               ...
               progress, evasion: Optional[Evasion] = None
               ) -> Optional[dict[str, list[Port]]]:
    """If raw mode is requested (or required by evasion) AND available, run it;
    else return None so the caller falls back to the connected-socket engine."""
    want_raw = prefer_raw or (evasion is not None and evasion.needs_raw)
    if not want_raw:
        return None
    try:
        ...
        return raw_udp_scan(ips, ports, timeout=max(timeout, 3.0),
                            ...
                            progress=progress, evasion=evasion)
...
def scan_udp(ip: str, ports: Optional[list[int]] = None, *,
    ...
    Args:
      ...
      evasion: Evasion config (source port, data-length, decoys, fragmentation).
    ...
    out = _scan_core([ip], ports, retries=retries, workers=workers, deep=deep,
                     ...
                     max_rate=max_rate, progress=progress, evasion=evasion)
    return out.get(ip, [])
...
def scan_udp_fast(ip: str, *, timeout: float = 1.2, retries: int = 2,
    ...
    return scan_udp(ip, udp_payloads.HIGH_VALUE_UDP_PORTS,
                    ...
                    deep=True, prefer_raw=prefer_raw, evasion=evasion,
                    progress=progress)
